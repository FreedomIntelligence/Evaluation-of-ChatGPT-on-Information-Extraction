...
## generate coarse_version
def generate_coarse_version(dataset, input_file, output_file):
    ...
    with open(out_file, 'w', encoding='utf-8') as fw:
        for example in data:
            for ent in example["entities"]:
                coarse_type = ent["e_type"].split("-")[0].strip()
                ent["e_type"] = coarse_type
                ent["e_type_verbose"] = coarse_type
...
if __name__ == "__main__":
    ...
    # generate coarse_version for fewnerd
    generate_coarse_version("ner/fewnerd/", "ner_test.json", "ner_test_coarse.json")
