...
def get_opts():
    ...
    parser.add_argument('--dataset', type=str, default="ner/conll03")
...
def ner_main(opts, bot):
    ...
    result_dir = os.path.join(opts.result_dir, opts.dataset)
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    
    input_dir = os.path.join(opts.input_dir, os.path.join(opts.dataset, opts.test_file))
    type_dir = os.path.join(opts.input_dir, os.path.join(opts.dataset, opts.type_file))
    print("loading data ...")
